...
class Node(object):
	...
	def getPayload(self):
		...
		return ret
	getTag = lambda self, name, attrs={}, namespace=None: self.getTags(name, attrs, namespace, one=1)
	...
	has_attr = lambda self, key: key in self.attrs
	""" Checks if node have attribute "key"."""
	__getitem__ = lambda self, item: self.getAttr(item)
	""" Returns node's attribute "item" value. """
	__setitem__ = lambda self, item, val: self.setAttr(item,val)
	""" Sets node's attribute "item" value. """
	__delitem__ = lambda self, item: self.delAttr(item)
...
class NodeBuilder:
	...
	stream_header_received = lambda self, ns, tag, attrs: self.check_data_buffer()
	""" Method called when stream just opened. """
	stream_footer_received = lambda self: self.check_data_buffer()
	""" Method called when stream just closed. """

	has_received_endtag = lambda self, level=0: self.__depth <= level and self.__max_depth > level
...
XML2Node = lambda xml: NodeBuilder(xml).getDom()
...
BadXML2Node = lambda xml: NodeBuilder(xml).getDom()
